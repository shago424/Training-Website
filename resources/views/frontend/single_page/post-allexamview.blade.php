            ...
            <div class="row" style="position: absolute;margin-top: -100px;margin-left: 400px;">
                <div class="col-lg-12 text-center">
                    <div class="breadcrumb__text">
                        <h2 style="color: white">সকল পরীক্ষার নোটিশ</h2>
        ...
        <thead>
            ...
            <tbody>
                    @foreach($exams as $key => $exam)
                      <tr>
                      <td>{{$key+1}}</td>
                      <td>{{$exam->post_title}}</td>
                      <td>{{date('d-M-Y',strtotime($exam->post_date))}}</td>
                      <td> <a class="btn btn-success btn-sm" href="{{asset('public/upload/postfile/'.$exam->post_file)}}" target="_blank">Download</a></td>
                      <td>
                    <a title="page" href="{{route('posts.page',$exam->id)}}" class="btn btn-primary btn-sm"><i class="fa fa-eye">View</i></a>
                      </td> 
                    </tr>
                    @endforeach
             </tbody>
